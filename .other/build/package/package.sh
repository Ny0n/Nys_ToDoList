...
if [ -z "$1" ]	# little safeguard, so that it doesn't do anything if we call it by double clicking on the script
then
	exit 1
fi
cd $(dirname $0) # second safeguard, we force the execution to be in the same folder the script is in
...
# Variables set via command-line options (some are set by default, just another safeguard!)
slug=
addonid=
topdir=
releasedir=$(pwd) # forced: same folder the script is in
overwrite=
nolib=
line_ending="dos"
skip_copying=
	...
	( cd "$_cdt_srcdir" && eval "$_cdt_find_cmd" ) | while read -r file; do
		...
		if [ -f "$_cdt_srcdir/$file" ]; then
			...
			if [ -z "$skip_copy" ] && match_pattern "$_cdt_check_file" "$_cdt_ignored_patterns"; then
				skip_copy="true"
				# CUSTOM ADDITION: here we ignore the changes done to the toc file, considering that my build is renaming it just before packaging.
				if [ $(basename "$file") == "$addonname.toc" ]; then
					skip_copy=
				fi
			fi
