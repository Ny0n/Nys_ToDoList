...
function database:ProfileChanged()
  ...
  if (NysTDL.db.profile.itemsDaily or NysTDL.db.profile.itemsWeekly or NysTDL.db.profile.itemsFavorite or NysTDL.db.profile.itemsDesc or NysTDL.db.profile.checkedButtons) then
    ...
    for catName, itemNames in pairs(oldItemsList) do -- for every cat we had
      ...
      for _, itemName in pairs(itemNames) do -- and for every item we had
        ...
        -- / tabName
        local tabName = "All"
        if (utils:HasItem(NysTDL.db.profile.itemsDaily, itemName)) then
          tabName = "Daily"
        elseif (utils:HasItem(NysTDL.db.profile.itemsWeekly, itemName)) then
          tabName = "Weekly"
        end
        -- / checked
        local checked = utils:HasItem(NysTDL.db.profile.checkedButtons, itemName)
        -- / favorite
        local favorite = nil
        if (utils:HasItem(NysTDL.db.profile.itemsFavorite, itemName)) then
          favorite = true
        end
        -- / description
