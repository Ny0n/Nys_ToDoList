...
local function createThemesTable()
  -- addon themes (rgb)
  database.themes = {
...
local function createDefaultsTable()
...
function database:DBInit(profileChanged)
  ...
  -- tabs resets
  resetManager:Initialize(profileChanged) -- TODO maybe change this to remove profileChanged here
...
function database:ProfileChanged(_, profile)
  ...
  -- first we reset what needs to be reset after a profile change,
  -- so that it can be reloaded after with the nex database
  mainFrame:ResetContent()

  self:DBInit(true) -- in case the selected profile is empty TODO: test self

  -- we update the changes for the visuals
...
function database:Initialize()
  -- first we create each of the database tables
  createThemesTable()
  createDefaultsTable()
  createOptionsTable()
